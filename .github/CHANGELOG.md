...
- 修复关闭未保存文件的报错问题
- 修改关闭文件时的弹窗样式
- 增加 windows nsis 的 32 位安装包
- 增加 windows 的 32 位和 64 位的 zip 便携包
- 增加 Linux 的 AppImage 安装包
- 增加 Linux 的 rpm 安装包
- 增加 Linux 的 tar.gz 便携包
- 增加 MAC 的 dmg 的 x64 安装包
- 增加 MAC 的 x64 和 arm64 的 zip 便携包

## 26.1.2

- 更换多语言方案: 一个语言一个包 --> 所有语言一个包，在设置里面进行设置
  - 阶段九：命令面板相关补充
  - 阶段十：输入@后显示的快捷键
  - 阶段十一：部分杂项
  - 阶段十二：对话框和警告消息（文件操作、文件系统监听、导出和打印、错误处理）
