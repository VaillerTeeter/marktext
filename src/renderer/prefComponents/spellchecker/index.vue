<template>
  <div class="pref-spellchecker">
    <h4>{{ $t('pref.spellchecker.title') }}</h4>
    <compound>
      <template #head>
        <bool
          :description="$t('pref.spellchecker.enable')"
      ...
      <template #children>
        <bool
          :description="$t('pref.spellchecker.hideUnderline')"
        ...
        <bool
          v-show="isOsx"
          :description="$t('pref.spellchecker.autoDetect')"
          :bool="true"
          :disable="true"
        ></bool>
        <cur-select
          v-show="!isOsx"
          :description="$t('pref.spellchecker.defaultLanguage')"
    ...
    <div v-if="isOsx && spellcheckerEnabled" class="description">
      {{ $t('pref.spellchecker.osxDescription') }}
    </div>

    <div v-if="!isOsx && spellcheckerEnabled">
      <h6 class="title">{{ $t('pref.spellchecker.customDictTitle') }}</h6>
      <div class="description">{{ $t('pref.spellchecker.customDictDesc') }}</div>
      <el-table
        :data="wordsInCustomDictionary"
        :empty-text="$t('pref.spellchecker.tableEmpty')"
        style="width: 100%"
      >
        <el-table-column prop="word" :label="$t('pref.spellchecker.tableWord')">
        </el-table-column>

        <el-table-column fixed="right" :label="$t('pref.spellchecker.tableOptions')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleDeleteClick(scope.row)" type="text" size="small" :title="$t('pref.spellchecker.delete')">
    ...
    handleSpellcheckerLanguage (languageCode) {
      this.ensureDictLanguage(languageCode)
        ...
        .catch(error => {
          log.error(error)
          notice.notify({
            title: this.$t('pref.spellchecker.errors.switchTitle'),
            type: 'error',
        ...
        ipcRenderer.invoke('mt::spellchecker-remove-word', selectedItem.word)
          .then(success => {
            if (success) {
              ...
              notice.notify({
                title: this.$t('pref.spellchecker.errors.removeTitle'),
                type: 'error',
                message: this.$t('pref.spellchecker.errors.removeMessage')
              })
            }
          })
